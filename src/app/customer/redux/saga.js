...
function* fetchCustomerActivitySaga({ payload }) {
  try {
    ...
    const response = yield call(
      api.get,
      `${ApiEndpoints.CUSTOMERS}/${payload}/activity`,
    );
    yield put(fetch_customer_activity_success(response.data));
  } catch (error) {
    const errorMessage =
      error.response?.data?.error || "Failed to fetch customer activity";
...
function* searchCustomersSaga({ payload }) {
  try {
    const response = yield call(api.get, `${ApiEndpoints.CUSTOMERS}/search`, {
