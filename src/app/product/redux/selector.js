...
export const getProducts = createSelector(productState, (state) => {
...
export const getError = createSelector(productState, (state) => state.ui.error);
...
export const getInstagramProducts = createSelector(getProducts, (products) => {
  if (!Array.isArray(products)) {
    console.warn("Products is not an array:", products);
    return [];
  }
  const filtered = products.filter(
    (p) => p?.type?.toLowerCase() === "instagram",
  );
  return filtered;
});

export const getFacebookProducts = createSelector(getProducts, (products) => {
  if (!Array.isArray(products)) return [];
  return products.filter((p) => p?.type?.toLowerCase() === "facebook");
});

export const getTwitterProducts = createSelector(getProducts, (products) => {
  if (!Array.isArray(products)) return [];
  return products.filter((p) => p?.type?.toLowerCase() === "twitter");
});

export const getWhatsappProducts = createSelector(getProducts, (products) => {
  if (!Array.isArray(products)) return [];
  return products.filter((p) => p?.type?.toLowerCase() === "whatsapp");
});

export const getYoutubeProducts = createSelector(getProducts, (products) => {
  if (!Array.isArray(products)) return [];
  return products.filter((p) => p?.type?.toLowerCase() === "youtube");
});

export const getTiktokProducts = createSelector(getProducts, (products) => {
  if (!Array.isArray(products)) return [];
...
export const getEscrowStatus = createSelector(productState, (state) => ({
  loading: state.ui.loading,
  error: state.ui.error,
  success: state.ui.success,
...
export const getPlatformStats = createSelector([productState], (state) => ({
